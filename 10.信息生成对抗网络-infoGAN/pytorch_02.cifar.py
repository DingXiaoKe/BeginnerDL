...
class Generator(Module):
    def __init__(self):
        ...
        self.convt3 = ConvTranspose2d(64, IMAGE_CHANNEL, kernel_size = 4, stride = 2, padding = 1)
    ...
    def forward(self, x):
        ...
        x = ReLU()(self.bn2(self.fc2(x))).view(-1, 256, 4, 4)

        x = ReLU()(self.bn3(self.convt1(x)))
        x = ReLU()(self.bn4(self.convt2(x)))
        x = self.convt3(x)
...
class Discriminator(Module):
    def __init__(self):
        ...
        self.conv1 = Conv2d(IMAGE_CHANNEL, 64, kernel_size = 4, stride = 2, padding = 1) # 32 x 32 -> 16 x 16
        self.conv2 = Conv2d(64, 128, kernel_size = 4, stride = 2, padding = 1) # 16 x 16 -> 8 x 8
        self.conv3 = Conv2d(128, 256, kernel_size=4, stride=2, padding=1) # 8 * 8 -> 4 * 4
    ...
    def forward(self, x):
        ...
        x = LeakyReLU()(self.bn1(self.conv2(x)))
        x = LeakyReLU()(self.bn2(self.conv3(x))).view(-1, self.fc1.in_features)

        x = LeakyReLU()(self.bn3(self.fc1(x)))
...
reader = DataReader(dataPath="/data/cifar10")
x_train, targets_train = reader.readData(phrase="train")
x_test, targets_test = reader.readData(phrase="test")
...
def sample_images(epoch):
    ...
    for i in range(c):
        ...
        for j in range(r):
            idx = i * 10 + j + 1
            axs[j,i].imshow(np.transpose(np.round(out_gen.cpu().data.numpy()[idx - 1]),(1,2,0)))
            axs[j,i].axis('off')
    fig.savefig("output/cifar_%03d.png" % epoch)
    plt.close()

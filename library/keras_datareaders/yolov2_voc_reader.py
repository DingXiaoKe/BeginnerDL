...
def read_data(filepath, target_image_size=(416, 416)):
    """
...
def data_generator(imageFile, batch_size=16, target_image_size=(416, 416)):
    """
...
def convert_ground_truth(box, image_size):
    ...
    """
    anchors_length = cfg.ANCHOR_LENGTH
    half = cfg.ANCHOR_VALUE / 2.
    ...
    object_mask = np.zeros((cfg.BOUX_CELL_SIZE, cfg.BOUX_CELL_SIZE, anchors_length, 1))
    object_value = np.zeros((cfg.BOUX_CELL_SIZE, cfg.BOUX_CELL_SIZE, anchors_length, 5))
    #object_mask = []
    #object_value = []

    box_wh = box[2:4]/np.array([width/cfg.BOUX_CELL_SIZE, height/cfg.BOUX_CELL_SIZE])#32 is for downsample factor 32, may change in other net structure
    half = box_wh / 2
    ...
    if maximum_iou>0:
        ...
        object_mask[np.int((box[0]+box[2]/2)/(width/cfg.BOUX_CELL_SIZE)), \
                    np.int((box[1]+box[3]/2)/(height/cfg.BOUX_CELL_SIZE)), index, 0] = 1
        #object_mask.append([np.floor(box[0]/32.), np.floor(box[1]/32.), index])
        object_value[np.int((box[0]+box[2]/2)/(width/cfg.BOUX_CELL_SIZE)), \
                     np.int((box[1]+box[3]/2)/(height/cfg.BOUX_CELL_SIZE)), index] = [x,y,w,h,box[4]]

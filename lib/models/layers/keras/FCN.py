from keras.layers import Input,Conv2D,BatchNormalization,Activation,MaxPooling2D,Lambda, Add
from keras.models import Model
from keras.regularizers import l2
from models.layers.keras.FastMaskRCNN import *
...
from models.losses.keras import fastmaskrcnn as losses
from datareader.keras import FastMaskRCNN as datareader
...
def FCN_Resnet50_16s(batch_shape=None):
    cfg = FCNConfig()
    if batch_shape:
        input = Input(batch_shape=(1,cfg.IMAGE_SIZE,cfg.IMAGE_SIZE,cfg.IMAGE_CHANNEL))
    else:
        input = Input(shape=(cfg.IMAGE_SIZE, cfg.IMAGE_SIZE, cfg.IMAGE_CHANNEL))

    bn_axis = 3
    network = Conv2D(filters=65, kernel_size=(7,7), strides =(2,2), padding='same', kernel_regularizer=l2(cfg.WEIGHT_DECAY))(input)
    network = BatchNormalization(axis=bn_axis, momentum=cfg.BATCH_MOMENTUM)(network)
    network = Activation('relu')(network)
    network = MaxPooling2D((3,3),(2,2))(network)

    network = conv_block(3, [64,64,256], strides=(1,1))(network)
    network = identity_block(3, [64,64,256])(network)
    network = identity_block(3, [64,64,256])(network)

    network = conv_block(3, [128, 128, 512])(network)
    network = identity_block(3, [128, 128, 512])(network)
    network = identity_block(3, [128, 128, 512])(network)
    network = identity_block(3, [128, 128, 512])(network)

    network = conv_block(3, [256, 256, 1024])(network)
    network = identity_block(3, [256, 256, 1024])(network)
    network = identity_block(3, [256, 256, 1024])(network)
    network = identity_block(3, [256, 256, 1024])(network)
    network = identity_block(3, [256, 256, 1024])(network)
    network = identity_block(3, [256, 256, 1024])(network)

    network = atrous_conv_block(3, [512, 512, 2048], atrous_rate=(2, 2))(network)
    network = atrous_identity_block(3, [512, 512, 2048],atrous_rate=(2, 2))(network)
    network = atrous_identity_block(3, [512, 512, 2048],atrous_rate=(2, 2))(network)

    network = Conv2D(cfg.NUM_CLASS, (1, 1), kernel_initializer='he_normal', activation='linear', padding='same', strides=(1, 1), kernel_regularizer=l2(cfg.WEIGHT_DECAY))(network)
    network = BilinearUpSampling2D(target_size=(cfg.IMAGE_SIZE, cfg.IMAGE_SIZE))(network)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # came from.
            _, _, _, active_class_ids = KL.Lambda(lambda x: utils.parse_image_meta_graph(x),
            ...
            mask_loss = KL.Lambda(lambda x: losses.mrcnn_mask_loss_graph(*x), name="mrcnn_mask_loss")(
    ...
    def compile(self, learning_rate, momentum):
        ...
        reg_losses = [l2(self.config.WEIGHT_DECAY)(w) / tf.cast(tf.size(w), tf.float32)
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        # Data generators
        train_generator = datareader.data_generator(train_dataset, self.config, shuffle=True,
                                         batch_size=self.config.BATCH_SIZE)
        val_generator = datareader.data_generator(val_dataset, self.config, shuffle=True,
                                       batch_size=self.config.BATCH_SIZE,
                                       augment=False)
    ...
    def run_graph(self, images, outputs):
        ...
        outputs = utils.OrderedDict(outputs)
        ...
        outputs_np = utils.OrderedDict([(k, v)
